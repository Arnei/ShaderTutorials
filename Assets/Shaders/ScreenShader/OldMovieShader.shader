Shader "CookbookShaders/OldMovieShader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
        	...
        	fixed4 frag(v2f_img i) : COLOR
        	{
        		...
        		half2 distortedUV = i.uv; //barrelDistortion(i.uv);
        		distortedUV = half2(i.uv.x, i.uv.y + (_RandomValue * _SinTime.z * 0.005));
        		fixed4 renderTex = tex2D(_MainTex, i.uv);

        		// Get the pixels rom the Vignette Texture
        		...
        		// Create a constant white color we can use to adjust the opacity of effects
        		fixed3 constantWhite = fixed3(1,1,1);

        		// Composite together the different layers to create the final Screen Effect
